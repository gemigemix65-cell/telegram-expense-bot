...
# ساختار داده جدید: اضافه شدن "income", "goals", "recurrences"
DEFAULT_DATA = {
    "expenses": [], 
    "income": [], 
    "categories": ["خوراک", "حمل و نقل", "تفریح", "سایر"],
    "goals": [], # {name: str, amount: float, saved: float, target_date: str, start_date: str}
    "recurrences": [] # {name: str, amount: float, category: str, frequency: str, last_run: str}
} 

# --- دسته‌بندی‌های هوشمند (برای حدس زدن دسته) ---
SMART_CATEGORIES = {
...
def check_and_run_recurrences():
    """بررسی و اجرای هزینه‌های تکراری که موعد آن‌ها رسیده است."""
    now = datetime.now()
...
def main_menu(message):
    ...
    buttons = [
        ...
        "/undo 🔙 حذف آخر",
        "/goal 🎯 هدف‌گذاری", # جدید
        "/recur 🔁 تکرار هزینه", # جدید
        "/addcat ➕ دسته‌بندی",
        "/setbudget 💰 بودجه",
        "/history 📜 تاریخچه", # جدید
        "/export 📤 خروجی CSV", # جدید
        "/tips 💡 پیشنهاد هوشمند",
        "/clear 🔄 پاکسازی"
    ]
    
    # چیدمان دکمه‌ها در ردیف‌های شکیل
...
def start(message):
    # اجرای چک کردن هزینه‌های تکراری در شروع
...
def handle_history_callback(call):
    bot.answer_callback_query(call.id, "در حال تولید گزارش تاریخی...")
    ...
    report_text, total_spent, chart_path = generate_report(filtered_expenses, f"ماه {month}/{year}", call.message)
    
    bot.edit_message_text(report_text, call.message.chat.id, call.message.message_id, parse_mode='Markdown', reply_markup=None)
...
def process_recur_step(message, frequency):
    ...
    try:
        ...
        data["recurrences"].append({
            ...
            "category": category, 
            "frequency": frequency, 
            "last_run": (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d") # برای اجرای اولیه
        })
        save_data()
...
# (توابع دیگر مانند /undo، /income، /tips و... همانند قبل هستند)
# (توابع handlerهای /setbudget، /addcat، /clear، add_expense_text، add_expense_voice و /filter نیز در جای خود قرار دارند و تغییرات لازم در آن‌ها اعمال شده است.)

# ... (ادامه توابع دستکاری نشده از کد قبلی) ...
# 
# 
# 3. اصلاح /report (با فیلتر تاریخ - قبلاً Request 2 بود)
...
def give_economic_advice(message):
    ...
    # 4. مشاوره هدف
    if data["goals"]:
        goal = data["goals"][0] # فقط هدف اول را بررسی می‌کنیم
        needed_monthly = (goal['amount'] - goal['saved']) / ((datetime.strptime(goal['target_date'], "%Y-%m-%d").year - datetime.now().year) * 12 + (datetime.strptime(goal['target_date'], "%Y-%m-%d").month - datetime.now().month))
...
if __name__ == '__main__':
    # این تابع در شروع کار ربات، هزینه‌های تکراری را اجرا می‌کند
    check_and_run_recurrences()
